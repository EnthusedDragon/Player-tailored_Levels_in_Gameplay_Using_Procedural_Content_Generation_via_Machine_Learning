...
public class MazePlayerHeuristic : Decision
{
    ...
    private MazePlayerAgent MazePlayerAgent;
    
    public override float[] Decide(List<float> vectorObs, List<Texture2D> visualObs, float reward, bool done, List<float> memory)
    {        
        if (MazePlayerArea == null)
        {
            MazePlayerArea = GameObject.FindGameObjectWithTag("HeuristicArea").GetComponent<MazePlayerArea>();
        }

        if (MazePlayerAgent == null)
        {
            MazePlayerAgent = GameObject.FindGameObjectWithTag("HeuristicAgent").GetComponent<MazePlayerAgent>();
        }
